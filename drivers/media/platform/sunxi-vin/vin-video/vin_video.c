					...
					struct v4l2_format *f)
{
	...
	f->fmt.win.w.left = vinc->vid_cap.frame.offs_h;
	f->fmt.win.w.top = vinc->vid_cap.frame.offs_v;
...
void __osd_bitmap2dram(struct vin_osd *osd, void *databuf)
{
	...
	void *dram_addr = osd->overlay_mask;
					...
					struct v4l2_format *f)
{
	...
	for (i = 0; i < osd->clipcount; i++) {
		osd->region[i].left = clip[i].c.left;
		osd->region[i].top = clip[i].c.top;
		osd->region[i].width = clip[i].c.width;
		osd->region[i].height = clip[i].c.height;
		bitmap_size += clip[i].c.width * clip[i].c.height;
		vin_log(VIN_LOG_FMT, "region: %d. %d. %d. %d\n",
	...
	if (f->fmt.win.bitmap) {
		...
		vin_log(VIN_LOG_FMT, "bitmap_size %d, pix_size %d, alpha %d\n",
	...
	if (osd->overlay_mask)
		kfree(osd->overlay_mask);
	osd->overlay_mask = kzalloc(bitmap_size * pix_size, GFP_KERNEL);
	if (osd->overlay_mask == NULL) {
		vin_err("%s - Alloc of bitmap buf failed\n", __func__);
		return -ENOMEM;
	}
...
static int __osd_reg_setup(struct vin_osd *osd, unsigned int on)
{
	return 0;
}
...
static int vidioc_overlay(struct file *file, void *__fh, unsigned int on)
{
	...
	ret = __osd_reg_setup(osd, on);
	osd->is_set = 1;
	kfree(osd->overlay_mask);
	osd->overlay_mask = NULL;
	return ret;
}
