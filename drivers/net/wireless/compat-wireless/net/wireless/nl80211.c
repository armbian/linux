...
#define NLA_PUT(a, b, c, d) do { \
	if (nla_put(a, b, c, d)) \
		goto nla_put_failure; \
	} while (0)

#define NLA_PUT_U8(a, b, c) do { \
	if (nla_put_u8(a, b, c)) \
		goto nla_put_failure; \
	} while (0)

#define NLA_PUT_U16(a, b, c) do { \
	if (nla_put_u16(a, b, c)) \
		goto nla_put_failure; \
	} while (0)

#define NLA_PUT_U32(a, b, c) do { \
	if (nla_put_u32(a, b, c)) \
		goto nla_put_failure; \
	} while (0)

#define NLA_PUT_U64(a, b, c) do { \
	if (nla_put_u64(a, b, c)) \
		goto nla_put_failure; \
	} while (0)

#define NLA_PUT_FLAG(a, b) do { \
	if (nla_put_flag(a, b)) \
		goto nla_put_failure; \
	} while (0)

#define NLA_PUT_STRING(a, b, c) do { \
	if (nla_put_string(a, b, c)) \
		goto nla_put_failure; \
	} while (0)
...
static int nl80211_dump_wiphy(struct sk_buff *skb, struct netlink_callback *cb)
{
	...
	list_for_each_entry(dev, &cfg80211_rdev_list, list) {
		...
		if (++idx <= start)
			continue;
		if (nl80211_send_wiphy(skb, NETLINK_CB_PORTID(cb->skb),
			     ...
			     struct netlink_callback *cb)
{
	...
	/*TES_P2P_0005 WorkAround:
	* p2p sd query frame will be sent after there are some p2p peers in p2p find result .
	* However  the result maybe too old to make sure these p2p peer is in p2p listen
	* then p2p peer can't receiver the p2p sd query frame.
	* cleaning the result after getting result make the result is new as WorkAround
	* but it will cause some channel can't experence in ui. For Example : Channel 11
	*/
...
static int nl80211_register_mgmt(struct sk_buff *skb, struct genl_info *info)
{
	...
	return cfg80211_mlme_register_mgmt(dev->ieee80211_ptr, genl_info_snd_portid(info),
			frame_type,
